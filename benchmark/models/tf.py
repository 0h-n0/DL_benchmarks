    ...
    def run(self, iterator, mode='train'):
        ...
        with tf.Session(config=self.config) as sess:
            ...
            for idx, (x, t) in enumerate(iterator):
                ...
                train_step.run(feed_dict={self.model.x: x, self.model.y_: t.T})
...
def cnn(x, channel, xdim, ydim, output_num):    
    ...
    with tf.name_scope('conv2'):
        W_conv2 = weight_variable([1, 3, 180, 180])
        b_conv2 = bias_variable([180])
        h_conv2 = tf.nn.relu(conv2d(h_conv1, W_conv2) + b_conv2)
      
    with tf.name_scope('pool1'):
        h_pool1 = max_pool(h_conv2, (1, 2), stride=(2, 2))

    with tf.name_scope('conv3'):
        W_conv3 = weight_variable([1, 3, 180, 180])
        b_conv3 = bias_variable([180])
        h_conv3 = tf.nn.relu(conv2d(h_pool1, W_conv3) + b_conv3)

    with tf.name_scope('conv4'):
        W_conv4 = weight_variable([1, 3, 180, 180])
    ...
    with tf.name_scope('conv5'):
        W_conv5 = weight_variable([1, 2, 180, 180])
        b_conv5 = bias_variable([180])
        h_conv5 = tf.nn.relu(conv2d(h_pool2, W_conv5) + b_conv5)

    with tf.name_scope('conv6'):
        W_conv6 = weight_variable([1, 1, 180, 180])
    ...
    with tf.name_scope('fc1'):
        ...
        h_conv6_flat = tf.reshape(h_conv6, [-1, 3 * 180])
